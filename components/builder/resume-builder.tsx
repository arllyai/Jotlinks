...
function TextInput(props: InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className="ms-input" />;
}

function TextArea(props: TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} className="ms-input" />;
}
    ...
    <SectionCard title="Section Order">
      ...
      <div className="space-y-2">
        {order.map((section, index) => (
          <div
            key={section}
            className="ms-card-soft flex items-center justify-between px-3 py-2"
          >
            ...
            <div className="flex gap-2">
              <button
                ...
                className="ms-btn-secondary px-2 py-1 text-xs disabled:opacity-40"
              >
                Up
              </button>
              <button
...
export function ResumeBuilder({
  ...
  const copyPublicLink = async () => {
    ...
    const targetUrl = `${getClientBaseUrl()}${publicUrl}`;
    try {
      await navigator.clipboard.writeText(targetUrl);
      setStatusMessage("Public link copied to clipboard.");
    } catch {
      setStatusMessage(`Copy failed. Here is your link: ${targetUrl}`);
    }
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <Link
            href="/dashboard"
            className="text-xs font-medium text-[var(--muted)] underline-offset-4 hover:underline"
          >
            ← Back to dashboard
          </Link>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="ms-btn-secondary px-3 py-1.5 text-xs font-medium transition disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          <button
            ...
            onClick={downloadPdf}
            disabled={!hasPaidAccess}
            className="ms-btn-primary px-3 py-1.5 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-60"
          >
            Download PDF
          </button>
        </div>
                  ...
                  <div className="mt-2">
                    <div className="mb-1 flex items-center justify-between gap-2">
                      ...
                      <button
                        ...
                        className="ms-btn-primary px-2 py-1 text-[11px] font-medium disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        {generatingId === entry.id
                          ? "Generating..."
                          : "Generate with AI"}
